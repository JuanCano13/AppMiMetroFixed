...
@CrossOrigin(origins = "*") // Permite peticiones desde el frontend HTML (importante para desarrollo)
public class FallaController {
    ...
    // Endpoint para el panel de alertas agrupadas (existente)
    @GetMapping("/conteo")
    ...
    // NUEVOS ENDPOINTS PARA LAS GRÁFICAS:
    ...
    @GetMapping("/graficas/congestion-por-hora")
    ...
    public List<Map<String, Object>> getTiposFalla() {
        List<Object[]> resultados = fallaService.obtenerConteoFallasPorTipo();
        List<Map<String, Object>> respuesta = new ArrayList<>();
        for (Object[] fila : resultados) {
            Map<String, Object> item = new HashMap<>();
